...
class siteThumbpagePlugin extends sitePlugin
{
    ...
    {
        $pageMmodel = new sitePageModel();
        $page = $pageMmodel->getById($post_id);
        if ($page['thumbpage']) {
            return wa()->getDataUrl('plugins/thumbpage/images/' . $page['thumbpage'], true, 'site');
        }
    }
    ...
    public static function plugin($pluginId = '')
    {
        ...
        if (!array_key_exists($pluginId, $plugin)) {
            $plugin[$pluginId] = wa('site')->getPlugin($pluginId);
        }
